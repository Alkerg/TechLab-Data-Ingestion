...
- **MQTT Broker Processor**: Componente encargado de recibir los datos  a través del ```MQTT Broker```. Para ello se crea un cliente MQTT que se suscribe a un **TOPIC** del ```MQTT Broker``` y reenvía los datos al ```Data Broker```.
